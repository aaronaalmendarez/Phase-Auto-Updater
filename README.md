...
`PhaseAnimatorSetup.exe` is the one-click installer to share with most Windows users. It carries the MSI inside it and starts the install flow automatically. The MSI installs per-user, adds desktop/start menu shortcuts, and does not need admin by default.
To start the updater on login:

```powershell
msiexec /i .\dist\windows\PhaseAutoUpdater-0.1.1.msi START_ON_LOGIN=1
```
...
- `release.yml` builds `PhaseAnimatorSetup.exe`, the Windows MSI, and the macOS zip when a tag like `v0.1.1` is pushed.
