        ...
        "capabilities": [
            {
                ...
                "status": "ready",
                "description": "Receives opt-in Phase Animator activity snapshots and publishes Discord Rich Presence through the local Discord client."
            },
            {
                "id": "presence",
...
fn run_server(
    ...
    let mut discord_presence = DiscordPresenceManager::new();
...
fn poll_clients(
    ...
    discord_presence: &mut DiscordPresenceManager,
) {
    let mut index = 0;
...
fn send_auto_reply(
    ...
    let reply_op = match packet.op.as_str() {
        ...
        "discord_presence.update" => "discord_presence.update.ok",
        "discord_presence.clear" => "discord_presence.clear.ok",
        _ => "ack",
    };
    let payload = match packet.op.as_str() {
        ...
        "discord_presence.update" => discord_presence.update(&packet.payload),
        "discord_presence.clear" => discord_presence.clear(),
