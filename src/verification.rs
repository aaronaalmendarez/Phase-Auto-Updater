...
#[derive(Clone, Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct CompanionBuildAccess {
    pub early_access: bool,
    pub status: String,
    pub early_access_release: Option<VersionResponse>,
}

impl CompanionBuildAccess {
    pub fn is_early_access(&self) -> bool {
        self.status == "verified" && self.early_access
    }

    pub fn downloadable_release(&self) -> Option<&VersionResponse> {
        self.early_access_release.as_ref().filter(|release| {
            self.is_early_access()
                && release.ok
                && release.download_available
                && !release.blocked
                && release.access_channel == "early-access"
                && !release.latest_build_id.is_empty()
                && release.release.as_ref().is_some_and(|info| {
                    info.personalized && info.build_id == release.latest_build_id
                })
        })
    }
}
...
#[cfg(test)]
mod companion_access_tests {
    use super::*;
    use serde_json::json;

    fn access(channel: &str, personalized: bool) -> CompanionBuildAccess {
        serde_json::from_value(json!({"earlyAccess":true,"status":"verified","earlyAccessRelease":{
            "ok":true,"product":"Phase Animator","latestVersion":"3.2.0","latestBuildId":"ea-1",
            "minimumBuildId":"","updateRequired":false,"required":false,"blocked":false,
            "downloadAvailable":true,"message":"","notes":"","accessChannel":channel,
            "release":{"buildId":"ea-1","version":"3.2.0","size":1,"sha256":"","createdAt":0,"personalized":personalized}
        }})).unwrap()
    }
    #[test]
    fn only_personalized_early_access_can_be_offered() {
        assert!(
            access("early-access", true)
                .downloadable_release()
                .is_some()
        );
        assert!(access("booster", true).downloadable_release().is_none());
        assert!(access("stable", true).downloadable_release().is_none());
        assert!(
            access("early-access", false)
                .downloadable_release()
                .is_none()
        );
        let mut revoked = access("early-access", true);
        revoked.early_access = false;
        assert!(!revoked.is_early_access());
        assert!(revoked.downloadable_release().is_none());
    }
    #[test]
    fn stale_blocked_or_mismatched_metadata_fails_closed() {
        for status in ["blocked", "unavailable", ""] {
            let mut value = access("early-access", true);
            value.status = status.into();
            assert!(!value.is_early_access());
            assert!(value.downloadable_release().is_none());
        }
        let mut value = access("early-access", true);
        value.early_access_release.as_mut().unwrap().blocked = true;
        assert!(value.downloadable_release().is_none());
        let mut value = access("early-access", true);
        value.early_access_release.as_mut().unwrap().latest_build_id = "other".into();
        assert!(value.downloadable_release().is_none());
    }
    #[test]
    fn old_account_response_cannot_grant_early_access() {
        let me: PluginMeResponse = serde_json::from_value(
            json!({"user":{"username":"tester"},"pluginLinked":true,"pluginSession":null}),
        )
        .unwrap();
...
pub struct PluginMeResponse {
    ...
    pub build_access: Option<CompanionBuildAccess>,
}
